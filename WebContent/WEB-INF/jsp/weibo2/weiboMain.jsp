...
<body>
	...
	<div id="container">
		...
		<%-- 	<c:set var="icount" value="${statusIndex.index }" />
 			<a href="./eventDetail?eid=${eventList[icount].eventId }">			</a> --%>

		<c:forEach items="${imageList }" var="img" varStatus="statusIndex">
			<img class="images" src="../public/images/${img }" />
		</c:forEach>

		<%-- 		
		        		<c:forEach items="${eventList }" var="event" varStatus="statusIndex">
		        							  <a href="./eventDetail?eid=${event.eventId }">
		        							  					  <c:set var="icount" value="${statusIndex.index }" />
		        							  			<img class="images" src="../public/images/${imageList[icount] }" />
		        						 	 </a>
		...
		<%-- 	<a href="./eventDetail?eid=${event.eventId }">			</a>
 --%>
		<c:forEach items="${eventList }" var="event" varStatus="statusIndex">
			<div class="event_intro" id="${statusIndex.index }"
				index="${event.eventId }">
